...

{
	...
	public function payment_detail($where="",$options=array())
	{
		$this->db->select('P.payment_id,U.first_name,U.last_name,U.email,E.event_name,P.txn_id,P.payment_gross,P.currency_code,P.payer_email,P.payment_status');
		$this->db->from('payments as P');
		if($where != ""){
			$this->db->where($where);
		}
		$this->db->join('user as U','U.id = P.user_id');
		$this->db->join('event_tbl as E','E.id = P.event_id');
	...
	public function get_user_by_lat($where,$user_latitude,$user_longitude)
	{
		
		$this->db->select('*, (
     3959 * acos (
      cos ( radians("'.$user_latitude.'") )
      * cos( radians( U.user_latitude ) )
      * cos( radians( U.user_longitude ) - radians("'.$user_longitude.'") )
      + sin ( radians("'.$user_latitude.'") )
      * sin( radians( U.user_latitude ) )
    )
	...
	public function get_user_list($where="",$options=array())

	{

		$this->db->select('U.id,U.username,U.phone,U.dob,U.gender,U.identity_tag,U.hobby,U.user_coin,U.user_image,U.whats_up,U.created_at,U.status,C.nicename,C.country_image');
		...
		$this->db->join('country as C','U.user_country = C.id');
	...
	public function event_list_data($where="",$options=array())
	{
		$this->db->select('E.id,E.status as event_status,E.event_live_status,E.user_id,E.event_name,E.event_latitude,E.event_longitude,E.event_venue,E.event_description,E.event_start_date,E.event_end_date,E.event_created_at,E.visibility_status,E.payment_status,JU.status,JU.event_owner_status');
		$this->db->from('event_tbl as E');
		if($where != ""){
			$this->db->where($where);
		}
		$this->db->join('event_join_user as JU','JU.event_id = E.id');
		$this->db->order_by("E.event_start_date",'DESC');
...

	{
		...
		$this->db->select('U.id,U.first_name,U.last_name,U.email,U.user_image,NT.message,NT.date,NT.type,NT.id as Notification_id,NT.note_id,NT.event_id');
	...
	public function get_eventList_by_lat($where,$minimumDistance,$maximumDistance,$user_latitude,$user_longitude)

	{
      ...
      * sin( radians( U.user_latitude ) )
		...
		$this->db->having('distance >="'.$minimumDistance.'" AND distance <= "'.$maximumDistance.'"');
